...
def _installed_apps_body_span(text: str, settings_path: Path) -> tuple[int, int] | None:
    ...
    Raises :class:`ScaffoldError` — ``invalid_input`` when the file doesn't parse,
    ...
    value = node.value
    # end_lineno/end_col_offset are Optional in the ast stubs but always set on
    # a parsed node.
    ...
    close_offset = line_starts[end_lineno - 1] + end_col_offset - 1
    return open_offset + 1, close_offset


def _last_import_line(text: str) -> int:
    """Return the 0-based index just after the last top-level import.

    Falls back to a line scan when *text* does not parse, which is the best
    available answer for a file that is already broken.
    """
    try:
        tree = ast.parse(text)
    except SyntaxError:
...
def ensure_installed_app(root: Path, app: str) -> bool:
    ...
    semantics, mirroring ``ensure_middleware``). Raises :class:`ScaffoldError`
...
def append_router_include(
    ...
    - Raises :class:`ScaffoldError` (``file_not_found``) when ``urls.py`` is
    ...
    if import_stmt is not None and import_stmt not in text:
        # Insert after the last top-level import statement. The position comes
        # from the AST so a parenthesized multi-line import counts as one
        # statement — a line scan would land inside its parentheses.
        at = _last_import_line(text)
        # The app imports form their own sorted block, separated by a blank
        # line from the framework ones. `apps` is the project's own package, so
        # an import sorter puts it in a later section: without the blank line
        # and the ordering, every scaffolded project fails its own lint.
        app_imports = [i for i, line in enumerate(lines[:at]) if line.startswith("from apps.")]
        if app_imports:
            position = next(
                (i for i in app_imports if lines[i] > import_stmt), app_imports[-1] + 1
            )
            lines.insert(position, import_stmt)
        else:
            lines[at:at] = ["", import_stmt]
...
# The project urls.py, used both by ``zeeb startproject`` and as the repair
# written by :func:`ensure_app_urls_included` when a project's urls.py is
# missing. It therefore carries no project name and imports nothing from
# ``apps.*`` — a hardcoded app import would fail at startup in any project that
# does not happen to have that app.
STANDARD_URLS_TEMPLATE = '''"""Project URL configuration.
...
router.include(create_auth_router(
    prefix=_AUTH_PREFIX,
    enable_registration=_REGISTRATION,
...
def ensure_app_urls_included(root: Path, app: str, prefix: str | None = None) -> bool:
    ...
    template first (repair semantics). Raises :class:`ScaffoldError` when the
