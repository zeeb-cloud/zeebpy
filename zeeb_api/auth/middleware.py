...
# Simple user object set on request.state (fallback when no DB lookup)
...
class JWTAuthMiddleware(BaseHTTPMiddleware):
    ...
    Usage:
        ...
        # With database user lookup (default)
        app.add_middleware(JWTAuthMiddleware, load_user_from_db=True)
        
        # Without database lookup (uses AuthenticatedUser from token claims)
        app.add_middleware(JWTAuthMiddleware, load_user_from_db=False)
        
        # With custom user loader
        async def my_loader(payload):
