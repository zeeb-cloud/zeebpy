...
| Tool | Description |
|---|---|
| `{prefix}activate_feature(feature, migrate=True, verify=True)` | Put an archived feature back — code, routes, and tests, exactly as they were. |
...
| `{prefix}delete_feature(feature, confirm=False, migrate=True, verify=True)` | Remove a feature and its data — the destructive one. |
...
| `{prefix}list_capabilities(include_docstrings=False, module=None, tier=None)` | Return a machine-readable inventory of every ``zeeb_agents`` tool. |
...
| Tool | Description |
|---|---|
| `{prefix}edit_feature(changes, app=None, migrate=True, verify=True, tests=True, feature=None)` | Edit an existing feature — alias of :func:`change_feature`. |
...
| `{prefix}alter_field(app, model_name, field)` | Redefine an existing field in place — type, length, nullability, default. |
...
| `{prefix}delete_serializer(app, model_name)` | Remove ``<ModelName>Serializer`` and its now-unused model import. |
| `{prefix}sync_serializer_field(app, model_name, field_name, present=True)` | Add or drop ONE field name in an existing serializer's ``Meta.fields``. |
...
| `{prefix}delete_viewset(app, model_name)` | Remove ``<ModelName>ViewSet`` and the imports only it used. |
...
| `{prefix}unregister_route(app, model_name)` | Remove a ViewSet's ``router.register(...)`` line and its import. |
